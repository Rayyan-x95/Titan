...
function SettingsSection({ title, className, children }: SettingsSectionProps) {
  return (
    <div className={cn('titan-section glass-panel p-6 border-white/5 space-y-2', className)}>
...
export function SettingsPage() {
  ...
  const tasksLength = useStore((state) => state.tasks.length);
  const notesLength = useStore((state) => state.notes.length);
  const expensesLength = useStore((state) => state.expenses.length);
  const {
    currency,
    compactMode,
    animations,
  ...
  const totalItems = tasksLength + notesLength + expensesLength;
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <SettingsSection title="Finance" className="!overflow-visible z-20">
          <SettingsRow
        ...
        <SettingsSection title="Data & Storage">
          ...
          <SettingsRow
            ...
            description="Synchronize all your tasks, notes, budgets, and finances directly between adjacent devices."
            action={
      ...
      <PeerSyncModal isOpen={isSyncOpen} onClose={() => setIsSyncOpen(false)} />
    </PageShell>
  );
}
